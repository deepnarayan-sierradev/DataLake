...
locals {
  ...
  state_machine_name = "${var.name_prefix}-extraction-workflow-${var.environment}"

  schedule_group_name = "${var.name_prefix}-extraction-schedules-${var.environment}"

  sfn_log_group_name = "/aws/vendedlogs/states/${var.name_prefix}-extraction-workflow-${var.environment}"

  pipeline_trigger_queue_name = "${var.name_prefix}-pipeline-trigger-${var.environment}.fifo"

  pipeline_trigger_lambda_name = "${var.name_prefix}-pipeline-trigger-${var.environment}"

  dlq_processor_lambda_name = "${var.name_prefix}-dlq-processor-${var.environment}"
}
...
resource "aws_cloudwatch_metric_alarm" "sfn_execution_failures" {
  alarm_name          = "${var.name_prefix}-extraction-workflow-failures-${var.environment}"
  ...
  tags = merge(local.common_tags, {
    Name = "${var.name_prefix}-extraction-workflow-failures-${var.environment}"
  })
}


resource "aws_cloudwatch_metric_alarm" "sfn_executions_throttled" {
  alarm_name          = "${var.name_prefix}-extraction-workflow-throttled-${var.environment}"
  ...
  tags = merge(local.common_tags, {
    Name = "${var.name_prefix}-extraction-workflow-throttled-${var.environment}"
  })
}
...
resource "aws_sqs_queue" "pipeline_trigger_dlq" {
  name                      = "${var.name_prefix}-pipeline-trigger-dlq-${var.environment}.fifo"
  fifo_queue                = true
...
resource "aws_cloudwatch_metric_alarm" "pipeline_trigger_dlq_depth" {
  alarm_name          = "${var.name_prefix}-pipeline-trigger-dlq-messages-${var.environment}"
  ...
  tags = merge(local.common_tags, {
    Name = "${var.name_prefix}-pipeline-trigger-dlq-messages-${var.environment}"
  })
}
...
resource "aws_sqs_queue" "async_dlq" {
  name                      = "${var.name_prefix}-orchestration-async-dlq-${var.environment}"
  message_retention_seconds = 1209600 # 14 days, the maximum — a DLQ that expires loses the evidence
  sqs_managed_sse_enabled   = true

  tags = merge(local.common_tags, { Name = "${var.name_prefix}-orchestration-async-dlq-${var.environment}" })
}
...
resource "aws_security_group" "orchestration_lambda" {
  ...
  name        = "${var.name_prefix}-orchestration-${var.environment}-sg"
  ...
  tags = merge(local.common_tags, { Name = "${var.name_prefix}-orchestration-${var.environment}-sg" })
