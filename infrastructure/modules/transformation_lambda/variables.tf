...
variable "execution_role_arn" {
  description = "ARN of the IAM role the Lambda assumes at runtime (transformation_runtime_role)."
  type        = string
}
...
variable "curated_s3_bucket_name" {
  description = "Name of the curated layer S3 bucket. Passed to Lambda as CURATED_S3_BUCKET env var."
  type        = string
}

variable "field_mapping_s3_bucket_name" {
  description = "Name of the bucket that holds field mapping JSON files. Passed as FIELD_MAPPING_S3_BUCKET. Typically same as curated_s3_bucket_name."
  type        = string
}
...
variable "glue_catalog_database" {
  description = "Optional Glue catalog database name for curated dataset registration. Empty string disables catalog registration."
  type        = string
  default     = ""
}
...
variable "cloudwatch_log_group_arn" {
  description = "ARN of the platform CloudWatch Log Group for transformation (creates implicit dependency on observability module)."
  type        = string
}
...
variable "memory_size_mb" {
  description = "Lambda memory allocation in MB. Transformation is memory-bound for large Parquet files."
  type        = number
  default     = 1024
  validation {
...
variable "timeout_seconds" {
  description = "Lambda timeout in seconds. Transformation of large entities may take several minutes."
  type        = number
  default     = 900
  validation {
