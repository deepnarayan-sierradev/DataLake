...
def record_load_failure(table_name: str, *, connection_error: bool = False) -> None:
    """
    Record a load failure, distinguishing a connection failure from a data failure.

    They have different remediations — a connection error is network or credential, a load
    error is schema or data — so they are separate metrics rather than one.
    """
    record_platform_metric(PlatformMetric.SERVING_STORE_LOAD_FAILURES, 1.0, EntityId=table_name)
    if connection_error:
        record_platform_metric(
            PlatformMetric.SERVING_STORE_CONNECTION_ERRORS, 1.0, EntityId=table_name
        )
...
class ServingStoreLoaderInterface(abc.ABC):
    ...
    def _finalize_load(
        ...
        record_platform_metric(
            PlatformMetric.SERVING_STORE_LOAD_ROWS, total_loaded, EntityId=table_name
        )
        record_platform_metric(
            PlatformMetric.SERVING_STORE_LOAD_DURATION_MS,
            max(
                0.0,
                (
    ...
    def apply_statements(
        self, statements: tuple[str, ...], tenant_code: str, table_name: str
    ) -> int:
        """
        Execute already-generated DDL/DCL against this tenant's container (DL-SERV-07).

        Used for row-level-security policies, which must be applied after the table exists.
        Statements are generated by `serving_store/view_generator.py` from allowlisted
        identifiers — this method never composes SQL from caller input, it only executes what the
        generator produced.

        Returns the number of statements executed so an inert application is visible rather than
        assumed. Idempotent by construction: the generated statements use
        `CREATE OR REPLACE` / `DROP ... IF EXISTS` so re-running a load re-asserts the policy.
        """
        if not statements:
            return 0
        credentials = self._retrieve_credentials()
        container_name = self._validate_and_resolve_container(table_name, (), tenant_code)
        connection = self._connect(credentials, container_name)
        executed = 0
        try:
