...
| `ARCH-10` | ✅ DONE (2026-07-10) | `entity_resolution/resolution_config/resolution_config_registry.py`'s `ResolutionConfigRegistry.load()`/`.publish()` now require `tenant_code` and prefix every S3 key with `{tenant_code}/entity-resolution/...`, mirroring `ARCH-1`. Also fixed a related latent bug found while implementing this: the in-process cache (the registry instance is reused across warm Lambda invocations for different tenants) was keyed only by `entity_type`/version, not `tenant_code` — tenant A's cached config object would otherwise have been served to tenant B's request on a warm container. `GoldenRecordPublisher.from_registry()` and `entity_resolution_pipeline_handler.py` updated to thread `tenant_code` through. Regression tests: `entity_resolution/tests/test_resolution_config_registry.py`, `tests/test_tenant_isolation.py::TestResolutionConfigRegistryIsolation` |
| `ARCH-11` | ✅ DONE (2026-07-10) | `transformation/field_mapping/field_mapping_registry.py`'s `FieldMappingRegistryClient.load_rule_set()`/`.publish_rule_set()` now require `tenant_code` and prefix every S3 key with `{tenant_code}/field-mappings/...`, same pattern as `ARCH-10`. `TransformationPipeline.execute()` now passes `ctx.tenant_code` through (already on `TransformationContext`, just never threaded into this call). `scripts/seed_field_mappings.py`/`scripts/seed_entity_resolution_configs.py` both gained a `--tenant-code` flag (default `"demo"`) for migrating existing config to the new tenant-prefixed paths. Regression tests: `transformation/tests/test_field_mapping_registry.py`, `tests/test_tenant_isolation.py::TestFieldMappingRegistryIsolation` |
| `ARCH-12` (new, 2026-07-08) | 🟡 PARTIAL | `ConfigurationRepositoryClient`'s DynamoDB backend has the same `PK=source_id/SK=entity_id` (no tenant dimension) pattern `WatermarkRepository` had — but the read-side `_enforce_tenant_match` guard means it manifests as a **409 Conflict blocking Tenant B's onboarding** if both tenants pick the same connector/entity, not a silent data leak (lower severity than the `WatermarkRepository` bug `ARCH-1` fixed). Still deferred — `ARCH-10`/`ARCH-11` are done, but this one is a separate DynamoDB-key fix (`tenant_scoped_key()` on `EdlEntityExtractionConfig`), not bundled with the S3-registry work |
...
| `ARCH-21` (new, 2026-07-10 — real bug, unrelated to tenant scoping) | ✅ DONE | Found while investigating a report that scheduled Salesforce runs for `salesforce-account`/`salesforce-contract` were re-extracting the full dataset every run instead of behaving incrementally. Root cause: `watermark_management/watermark_repository/watermark_repository.py::advance_watermark()` set `last_successful_watermark = current.upper_watermark` (the *previous* run's own boundary) instead of `new_upper_watermark` (*this* run's confirmed boundary) — since `build_incremental_select()` bounds the query on both sides, this left every entity's committed watermark one run behind where it actually finished, widening the window for an extra cycle after every watermark transition (first-run bootstrap, checkpoint resume). Verified live against dev DynamoDB/Step Functions state; the mechanism reproduced identically across all 4 Salesforce entities and both MySQL RDS entities (no CloudTrail data-events trail exists in this account, so an external trigger for the account/contract instance specifically couldn't be confirmed, but wasn't needed to explain the pattern). Fixed: `advance_watermark()` now sets `last_successful_watermark = new_upper_watermark`. Regression test: `watermark_management/tests/test_watermark_repository.py::TestAdvanceWatermark::test_advance_bumps_version_and_last_successful` |
...
> current status — as of 2026-07-10, tenant data isolation and data separation are code-complete
> for every guaranteed-collision resource, including the previously-deferred `ARCH-10`/`ARCH-11`
> registry work (done 2026-07-10). `ARCH-12`–`ARCH-14` remain deliberately deferred
> (interleaved-but-non-colliding audit artifacts, or lower severity than `ARCH-1` — see their
> entries above) and `SEC-2` (IAM enforcement) is the only genuinely open item in this row.
