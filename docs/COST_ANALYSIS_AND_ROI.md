...
| **AWS Glue Catalog** | Data catalog entries; no compute cost | $0 | Only catalog storage; query compute runs on Athena. Note: the `edl_curated` Glue database is provisioned but not wired to any Lambda in dev today, so it stays empty regardless of volume (`docs/PLATFORM_STATUS.md`, Glue Catalog section) |
...
| **AWS Glue Data Catalog** | Catalog entries | Catalog-only cost (no Glue ETL jobs used). `edl_curated` database is provisioned but unwired to any Lambda in dev, so it never registers tables regardless of usage. | $0 |
