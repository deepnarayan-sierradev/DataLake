...
class TestRoundTrip:
    def test_a_key_survives_encode_then_decode(self) -> None:
        ...
        token = cp._encode_page_token(key)
        assert token is not None
        assert cp._decode_page_token(_event(token), "demo") == key

    def test_no_cursor_encodes_to_no_token(self) -> None:
        # The last page must not advertise a next page.
        assert cp._encode_page_token(None) is None
        assert cp._encode_page_token({}) is None

    def test_absent_token_decodes_to_none(self) -> None:
        assert cp._decode_page_token(_event(None), "demo") is None

    def test_token_is_opaque(self) -> None:
        token = cp._encode_page_token({"tenant_code": "demo", "sk": "company#c-1"})
        assert token is not None
...
class TestCrossTenantForgeryIsRejected:
    def test_a_key_naming_another_tenant_is_refused(self) -> None:
        ...
        forged = _forge(f"{cp._PAGE_TOKEN_PREFIX}{json.dumps({'tenant_code': 'other'})}")
        with pytest.raises(ValidationFailedError, match="does not belong to this tenant"):
            cp._decode_page_token(_event(forged), "demo")
    ...
    def test_a_key_without_a_tenant_is_allowed_through(self) -> None:
        ...
        token = _forge(f"{cp._PAGE_TOKEN_PREFIX}{json.dumps({'sk': 'company#c-1'})}")
        assert cp._decode_page_token(_event(token), "demo") == {"sk": "company#c-1"}
...
class TestMalformedTokensAre400s:
    ...
    def test_rejected_rather_than_restarting_from_zero(self, token: str) -> None:
        ...
        with pytest.raises(ValidationFailedError):
            cp._decode_page_token(_event(token), "demo")
    ...
    def test_a_bare_offset_is_no_longer_accepted(self) -> None:
        ...
        with pytest.raises(ValidationFailedError):
            cp._decode_page_token(_event(_forge("edl-page:50")), "demo")
