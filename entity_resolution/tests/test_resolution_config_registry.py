...
def _seed_company(client) -> None:
    ...
    _put(
        client,
        "acme-corp/entity-resolution/company/latest.json",
        {"match_rules_version": "v1", "survivorship_version": "v1"},
    )


def _seed_person(client) -> None:
    _put(client, "acme-corp/entity-resolution/person/match_rules_v1.json", _PERSON_MATCH_RULES)
    _put(client, "acme-corp/entity-resolution/person/survivorship_v1.json", _PERSON_SURVIVORSHIP)
    _put(
        client,
...
def test_load_person_survivorship_output_fields(s3_client):
    ...
    registry = ResolutionConfigRegistry(s3_bucket=_BUCKET, region_name=_REGION)
    config = registry.load("person", _TENANT)
...
def test_latest_pointer_resolves_version(s3_client):
    ...
    registry = ResolutionConfigRegistry(s3_bucket=_BUCKET, region_name=_REGION)
    config = registry.load("company", _TENANT)  # "latest" → "v1" via pointer
...
def test_explicit_version_bypasses_pointer(s3_client):
    ...
    registry = ResolutionConfigRegistry(s3_bucket=_BUCKET, region_name=_REGION)
    config = registry.load("company", _TENANT, match_rules_version="v1", survivorship_version="v1")
    assert config.match_rule_set.rule_set_version == "v1"
...
def test_not_found_for_missing_entity_type(s3_client):
    ...
    with pytest.raises(ResolutionConfigNotFoundError, match="unknown-entity"):
        registry.load("unknown-entity", _TENANT)


def test_parse_error_for_malformed_match_rules(s3_client):
    _put(
        s3_client, "acme-corp/entity-resolution/bad-entity/match_rules_v1.json", {"not_valid": True}
    )
    _put(
        s3_client,
        "acme-corp/entity-resolution/bad-entity/survivorship_v1.json",
        _PERSON_SURVIVORSHIP,
    )

    registry = ResolutionConfigRegistry(s3_bucket=_BUCKET, region_name=_REGION)
    with pytest.raises(ResolutionConfigParseError):
        registry.load("bad-entity", _TENANT, match_rules_version="v1", survivorship_version="v1")
...
def test_parse_error_for_unknown_match_strategy(s3_client):
    ...
    _put(s3_client, "acme-corp/entity-resolution/company/match_rules_v1.json", bad_rules)
    _put(
        s3_client, "acme-corp/entity-resolution/company/survivorship_v1.json", _COMPANY_SURVIVORSHIP
    )
    ...
    with pytest.raises(ResolutionConfigParseError, match="magic"):
        registry.load("company", _TENANT, match_rules_version="v1", survivorship_version="v1")
...
def test_parse_error_when_attribute_rule_field_not_in_output_fields(s3_client):
    ...
    _put(s3_client, "acme-corp/entity-resolution/company/match_rules_v1.json", _COMPANY_MATCH_RULES)
    _put(s3_client, "acme-corp/entity-resolution/company/survivorship_v1.json", bad_sv)
...
def test_unsafe_entity_type_rejected(s3_client):
    ...
    with pytest.raises(ValueError, match="entity_type"):
        registry.load("../etc/passwd", _TENANT)
...
def test_publish_writes_three_s3_objects(s3_client):
    ...
    keys_in_bucket = {
        ...
        for obj in s3_client.list_objects_v2(
            Bucket=_BUCKET, Prefix="acme-corp/entity-resolution/company/"
        )["Contents"]
    }
    assert mr_key in keys_in_bucket
    assert sv_key in keys_in_bucket
    assert "acme-corp/entity-resolution/company/latest.json" in keys_in_bucket
...
def test_publish_latest_pointer_contains_version(s3_client):
    ...
    pointer_body = s3_client.get_object(
        Bucket=_BUCKET, Key="acme-corp/entity-resolution/company/latest.json"
    )["Body"].read()
...
def test_load_json_parse_error_on_invalid_bytes(s3_client):
    ...
    s3_client.put_object(
        Bucket=_BUCKET,
        Key="acme-corp/entity-resolution/company/match_rules_v1.json",
...
def test_survivorship_with_no_output_fields_skips_validation(s3_client):
    ...
    _put(s3_client, "acme-corp/entity-resolution/company/match_rules_v1.json", _COMPANY_MATCH_RULES)
    _put(s3_client, "acme-corp/entity-resolution/company/survivorship_v1.json", passthrough_sv)
